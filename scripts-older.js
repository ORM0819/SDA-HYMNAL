document.addEventListener('DOMContentLoaded', function () {
    ...
    searchInput.addEventListener('input', function () {
        ...
        var filteredSongs = allSongs.filter(function (song) {
            var normalizedTitle = song.title.toLowerCase().replace(/[áàäâã]/g, 'a')
                                                      .replace(/[éèëê]/g, 'e')
                                                      .replace(/[íìïî]/g, 'i')
                                                      .replace(/[óòöôõ]/g, 'o')
                                                      .replace(/[úùüû]/g, 'u')
                                                      .replace(/[ñ]/g, 'n')
                                                      .replace(/[^\w\s]/g, ''); // Remove punctuation
            var normalizedNumber = song.number.toLowerCase();
            return normalizedNumber.indexOf(query) !== -1 || normalizedTitle.indexOf(query) !== -1;
        });
